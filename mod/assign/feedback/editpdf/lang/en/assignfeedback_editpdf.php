...
$string['errorgstimeout'] = 'GhostScript timed out after {$a->timeout} seconds while processing "{$a->file}". The PDF may be too complex or high-resolution to process within the configured time limit.';
...
$string['gstimeout'] = 'GhostScript processing timeout';
$string['gstimeout_desc'] = 'Maximum number of seconds to allow GhostScript to run when converting a single PDF page to an image. Set to 0 to disable the timeout. Default: 0 (disabled). Requires the GNU timeout utility (coreutils); silently ignored on Windows or if not found at /usr/bin/timeout or /bin/timeout.';
